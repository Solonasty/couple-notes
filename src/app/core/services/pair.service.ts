...
export class PairService {
  ...
  async createPairByEmail(partnerEmailRaw: string): Promise<void> {
    ...
    // (опционально, но полезно) если у тебя в UI что-то пошло не так:
    // не даём отправлять приглашение если уже в паре
    // const myRef = doc(this.fs, `users/${me.uid}`);
    // const mySnap = await getDoc(myRef);
    // if ((mySnap.data() as any)?.pairId) throw new Error('Вы уже состоите в паре');
    ...
    await setDoc(inviteRef, invite); // без merge
  }
